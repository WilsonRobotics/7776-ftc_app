...
class CameraDriveStep1 extends AutoLib.Step {
    ...
    public boolean loop() {
        ...
        String scanline = frame.scanlineDomColor(frame.cameraSize().height/2, mBandSize);
        ...
        for (int i=0; i<scanline.length(); i++) {
            ...
            if (m.matches() && m.groupCount() == 1) {
                ...
                patternSize = group1.length();
                patternStart = scanline.indexOf(group1);
                patternCenter = patternStart + patternSize / 2;
...
public class CameraDriveTest1Op extends OpMode {
    ...
    public void init() {
        ...
        mSequence.add(new CameraDriveStep1(this, mCamAcqFr, "r+(g+)b+[^b]*.*", mMotors, 0.5f, 5, 6));

        // add a second camera-directed Step that backs away until the pattern is <2 bands of 6 pixels
        mSequence.add(new CameraDriveStep1(this, mCamAcqFr, "r+(g+)b+[^b]*.*", mMotors, -0.5f, 2, 6));
